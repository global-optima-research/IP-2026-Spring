...
"""
Configs for ECT (Enhanced Consistency Training) on Wan2.1-1.3B.

ECT = CM with use_cd=False (no teacher, pure consistency training).
Adapted from EDM2/config_cm_s.py + WanT2V/config_mf.py (same consistency family).
...
def create_config():
    ...
    # Scale kimg_per_stage for video: smaller batch → slower curriculum
    # EDM2 uses 3200 with batch_size=1024; we use batch_size_global=8
    # Scale: 3200 * 1024 / 8 = 409600, but cap at reasonable level
    # Use a moderate value since we have fewer iterations (6000 vs 150000)
    ...
    # === EMA ===
    # Use power EMA (same as MeanFlow WanT2V config)
    ...
    # ECT: no teacher needed (use_cd=False is the default)
    ...
    # Optimizer (reference: MeanFlow WanT2V)
    ...
    # Time sampling: use logitnormal (same as MeanFlow WanT2V for flow matching)
    ...
    config.log_config.group = "wan_cm_ct"
    return config
