...
> 14 人团队（4 管理 + 10 研发），3-3-4 模块化分配 | 预计周期 16 周
...
| 角色 | 成员 | 职责 |
|------|------|------|
| **总体负责人** | 徐昊、李志颖 | 项目整体规划、进度把控、跨组协调、资源调配 |

| 小组 | Leader | 职责 |
|------|--------|------|
| **Task 1** 数据集构建 | 魏子恒、Jacky | 数据集构建方向指导、质量把关、进度跟踪 |
| **Task 2** Teacher Model 训练 | 李志颖、Jacky | 模型架构决策、训练策略指导、实验方向把控 |
| **Task 3** DMD 蒸馏加速 | 李志颖、Jacky | 蒸馏方案选型、技术路线指导、复现进度跟踪 |
...
| 模块 | 任务 | Leader | 成员 | 人数 |
|------|------|--------|------|------|
| **Task 1** | 数据集构建 | 魏子恒、Jacky | 王洁怡、刘麓琰、王志铭 | 3 |
| **Task 2** | Teacher Model 训练 | 李志颖、Jacky | 钟蕊、肖泓锐、方子维 | 3 |
| **Task 3** | DMD 蒸馏加速 | 李志颖、Jacky | 李一佳、陈庆展、Sze Long、邱张西子 | 4 |

---

## Task 1：数据集构建（3 人）

> **小组 Leader：魏子恒、Jacky** — 负责数据集构建方向指导、质量把关、进度跟踪
...
> **小组 Leader：李志颖、Jacky** — 负责模型架构决策、训练策略指导、实验方向把控
...
> **小组 Leader：李志颖、Jacky** — 负责蒸馏方案选型、技术路线指导、复现进度跟踪
